...
class ASRServer:
    ...
    def load_model(self):
        ...
        # shift_size[1] = frames to advance per step (8 for parakeet-tdt, 16 for nemotron)
        self.shift_frames = scfg.shift_size[1] if isinstance(scfg.shift_size, list) else scfg.shift_size

        # chunk_size[1] = full chunk size needed for first inference step
        self.chunk_frames = scfg.chunk_size[1] if isinstance(scfg.chunk_size, list) else scfg.chunk_size
    ...
    async def _handle_audio(self, session: ASRSession, audio_bytes: bytes):
        ...
        # First chunk needs chunk_frames; subsequent chunks need shift_frames of new data
        if session.emitted_frames == 0:
            min_audio_for_chunk = (self.chunk_frames + 1) * self.hop_samples
        else:
    ...
    def _process_chunk(self, session: ASRSession) -> Optional[str]:
        ...
        try:
            ...
            with torch.inference_mode():
                ...
                is_first_chunk = session.emitted_frames == 0
                if is_first_chunk:
                    # First chunk: need chunk_frames worth of mel frames
                    if available_frames < self.chunk_frames:
                        return session.current_text  # Not enough frames yet
                    chunk_start = 0
                    chunk_end = self.chunk_frames
                    drop_extra = 0
                else:
                    # Subsequent chunks: need shift_frames of new data, but send full chunk_frames
                    if new_frame_count < self.shift_frames:
                        return session.current_text  # Not enough new frames
                    # Sliding window: chunk_frames ending at emitted + shift
                    chunk_end = session.emitted_frames + self.shift_frames
                    chunk_start = max(0, chunk_end - self.chunk_frames)
                ...
                if is_first_chunk:
                    session.emitted_frames = self.chunk_frames
                else:
    ...
    def _process_final_chunk(self, session: ASRSession) -> Optional[str]:
        ...
        try:
            ...
            with torch.inference_mode():
                ...
                # Pad to minimum chunk_frames if needed (short utterances)
                if chunk_mel.shape[-1] < self.chunk_frames:
                    pad_size = self.chunk_frames - chunk_mel.shape[-1]
                    chunk_mel = torch.nn.functional.pad(chunk_mel, (0, pad_size), value=0.0)
