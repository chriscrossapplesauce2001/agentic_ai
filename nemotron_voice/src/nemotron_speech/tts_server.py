...
async def load_model(model_id: str = "nvidia/magpie_tts_multilingual_357m"):
    ...
    async with _model_lock:
        ...
        def _load():
            ...
            # Use January 2026 model revision (compatible with NeMo 2.7.0rc0 in container)
            nemo_path = os.environ.get("MAGPIE_TTS_NEMO_PATH", None)
            if nemo_path and os.path.isfile(nemo_path):
                logger.info(f"Loading from local .nemo file: {nemo_path}")
