...
- **Exercise 1 (free-text answers)** — graded by a **judge LLM**, one question at a time, against `rubric.json`. Default judge model: `nemotron-3-super:latest` (override via `JUDGE_MODEL` env var). Students tag each answer with its rubric id (`P<part>.Q<n>:`); the grader extracts the text after each tag. (Exercise 0 is ungraded LLM-basics onboarding.)
- **Exercise 2 (code TODOs)** — checked **deterministically** with anchored regexes on the `run_agent` code, no LLM.
